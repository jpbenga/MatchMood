...
import { logOutOutline, cameraOutline, videocamOutline, trashOutline, sendOutline, searchOutline, peopleOutline, peopleCircleOutline } from 'ionicons/icons';
...
export class HomePage implements OnInit {
  ...
  isUploading = false;
  isLoadingVibe = false;
  enrichedPosts$!: Observable<EnrichedPost[]>;
  vibeText: string = '';
  ...
  ngOnInit() {
    this.enrichedPosts$ = this.postService.getRecentPosts().pipe(
        switchMap(posts => {
            if (posts.length === 0) {
                return of([]);
            }
            const authorIds = [...new Set(posts.map(post => post.userId))];
            const authorProfileObservables = authorIds.map(authorId =>
                this.userService.getUserProfile(authorId).pipe(
                    filter(p => p !== undefined),
                    take(1)
                )
            );
            return forkJoin(authorProfileObservables).pipe(
                map(authorProfiles => {
                    const authorMap = new Map<string, User>();
                    authorProfiles.forEach(profile => {
                        if (profile) {
                            authorMap.set(profile.uid, profile);
                        }
                    });
  ...
  async sendVibe() {
      ...
      const postData: NewPostData = {
          type: 'vibe', textContent: this.vibeText.trim(),
          mediaUrl: null, filePath: null
      };
      try {
          const postId = await this.postService.addPost(postData);
          await loading.dismiss(); this.isLoadingVibe = false;
          if (postId) {
              this.vibeText = '';
              await this.presentToast('Vibe envoyée !', 'success');
          } else { throw new Error("La création du post vibe n'a pas retourné d'ID."); }
      } catch (error: any) {
          await loading.dismiss(); this.isLoadingVibe = false;
  ...
  async capturePhoto() {
    ...
    } else { this.capturedImage = null; }
  }

  async uploadPhotoAndCreatePost(photo: Photo) {
    this.isUploading = true; this.isLoadingVibe = false;
    ...
    try {
      ...
      const postData: NewPostData = {
        type: 'photo', mediaUrl: uploadResult.downloadUrl,
        filePath: uploadResult.filePath, textContent: null
      };
      const postId = await this.postService.addPost(postData);
      await loading.dismiss(); this.isUploading = false; this.capturedImage = null;
      if (postId) { await this.presentToast('Photo publiée avec succès !', 'success'); }
      else { throw new Error("La création du post photo n'a pas retourné d'ID."); }
    } catch (error: any) {
      await loading.dismiss(); this.isUploading = false;
  ...
  async captureVideo() {
      ...
      if (mediaFile) { await this.uploadVideoAndCreatePost(mediaFile); }
  }

  async uploadVideoAndCreatePost(mediaFile: MediaFile) {
      this.isUploading = true; this.isLoadingVibe = false; this.capturedImage = null;
      ...
      try {
          ...
          const postData: NewPostData = {
              type: 'short', mediaUrl: uploadResult.downloadUrl,
              filePath: uploadResult.filePath, textContent: null
          };
          const postId = await this.postService.addPost(postData);
          await loading.dismiss(); this.isUploading = false;
          if (postId) { await this.presentToast('Vidéo publiée avec succès !', 'success'); }
          else { throw new Error("La création du post vidéo n'a pas retourné d'ID."); }
      } catch (error: any) {
          await loading.dismiss(); this.isUploading = false;
  ...
  async confirmDeletePost(post: Post) {
      ...
      if (this.authService.currentUser?.uid !== post.userId) {
          await this.presentToast("Vous ne pouvez pas supprimer ce post.", 'danger'); return;
      }
      const alert = await this.alertCtrl.create({
          header: 'Confirmer', message: 'Voulez-vous vraiment supprimer ce post ?',
          buttons: [ { text: 'Annuler', role: 'cancel' }, {
                  ...
                  handler: async () => {
                      ...
                      try {
                          ...
                          await loading.dismiss(); await this.presentToast('Post supprimé.', 'success');
                      } catch (error: any) {
  ...
  goToFindFriends() { this.router.navigate(['/find-friends']); }
  goToFriendRequests() { this.router.navigate(['/friend-requests']); }
  goToFriendList() { this.router.navigate(['/friend-list']); }
  ...
  async presentToast(message: string, color: 'success' | 'danger' | 'medium' = 'danger') {
    const toast = await this.toastCtrl.create({ message: message, duration: 2500, position: 'bottom', color: color });
    toast.present();
  }
}